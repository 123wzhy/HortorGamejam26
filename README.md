...
启动后先进入主界面：选曲卡真实列出 `NEON GRID / ORIGINAL DEMO` 与 `GOLDEN STAMPEDE / ORIGINAL DEMO`。点击整行切换当前曲目，黄底行即当前选择；点击左侧圆形按钮通过 `Builda.audio.playBGM()` 试听，再点同位置的暂停按钮停止。右侧 1–3 星不是设计稿示例值，而是 `BeatmapDifficulty` 对已生成谱面的密度、BPM、方向变化和短间隔比例分析所得，当前两首分别为 2 星与 3 星。“帮助”展示判定规则；“排行榜”展示本设备前 5 条完整结算成绩与最近一次完整结算名次，并明确说明它不是平台排名；“设置”在 Builda 宿主内打开平台统一的暂停/设置/退出页。玩法右上可重新开始或返回主页。
...
两首 Demo 谱面都包含 8 个确定性组合、共 31 个 note。`NEON GRID` 为 100 BPM、每拍 `600ms`；`GOLDEN STAMPEDE` 为 120 BPM、每拍 `500ms`。试听文件由 `tools/generate-song-preview.mjs` 按各自 BPM 生成 16 拍原创 PCM 循环，时长分别为 `9.6s` 与 `8.0s`，与谱面/曲目 ID 一一映射。当前完整玩法仍锁定第一首歌：其谱面歌曲时间约 25 秒，组间舞段不消耗歌曲时间，因此包含完整 26.8 秒舞蹈的一轮约 52 秒。四种结果与单键基础分为：
...
assets/scripts/
  ...
  domain/SongCatalog.ts         双曲目稳定 ID、谱面、试听资源与动作映射
...
`SequenceEngine`、`GroupDanceFlow`、`LocalLeaderboard`、`JudgeSystem`、`TimingProgress`、`RhythmLayout`、`BeatmapDifficulty`、`SongPreviewController` 和 `UiStartupState` 不引用 `cc`，测试直接编译同一份 TypeScript 实现，避免维护一套与游戏实现漂移的 JS 镜像。测试还覆盖双曲目稳定 ID/试听映射、两档生成谱面难度、难度上下界、试听播放/暂停、快速切歌与 Play→Pause 异步竞态、宿主拒绝，以及 WAV 格式、时长、峰值、非静音和确定性哈希；原有判定、组合推进、榜单、布局与启动门控回归继续保留。
...
`BuildaAdapter` 的 `playBGM / stopBGM / playSFX` 契约只有在 SDK Result 的 `ok` 为 `true` 时才报告成功。选曲页现在通过 `SongPreviewController` 调用 `playBGM / stopBGM`，并以串行 operation id 防止快速切歌或 Play→Pause 后旧异步调用把声音重新打开。两段试听位于 `assets/audio/bgm/`，发布时必须以 `audio/**` 相对路径进入 Builda 外置资源包，不会复制进 H5 主包；游戏内不另造音乐/音效开关，静音继续由 Builda 平台通用设置统一管理。
...
- 选曲页已有两段按实际 BPM 生成的原创循环试听，但它们不是完整制作版 BGM，也不驱动 `SongClock`；正式音乐时长、音频时钟同步和 SFX 尚未接入。
- 两首歌的菜单选择、试听资源、难度和动作目录均已就绪；本阶段按并行开发边界没有改动 `SequenceEngine / GroupDanceFlow / DancerAnimationController`，因此完整玩法仍只启动 `NEON GRID`。选中第二首后会明确提示玩法切换留给后续 Phase B，而不会暗中运行第一首谱面。
- 3D 舞者 Bundle 已含 7 段状态动画；`gameplayDancer` 2D 备选图及仍未使用的附加玩法贴图等待各自 UI 迭代，后续进度轨可复用 `stonePanel` 九宫格。
